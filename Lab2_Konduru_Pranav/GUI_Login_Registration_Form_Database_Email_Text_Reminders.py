...
Twilio_Number="+12184133007", #this is your own Twilio number
account_sid = 'AC950f0d9b4bad5c4d41b010cb4f6a7f1f'  #this is your account SID
auth_token = '7a6b57625cd615e6586c7aeb89a1fb88'     #this is your own auth_token

from_address = "pinkearser432@gmail.com" #this is your own gmail account
app_password = 'mfcsurdfwxavqsuv' # a token for gmail, this is the app password from Gmail Security
...
def excel():
    ...
    sheet.cell(row=1, column=10).value = "InContact"
    sheet.cell(row=1, column=11).value = "Travel"
    sheet.cell(row=1, column=12).value = "HaveSymptoms"
    sheet.cell(row=1, column=13).value = "Sent"
    sheet.cell(row=1, column=14).value = "SendText"
...
#set focus on the the close_contact_field box
def focus9(event):
    close_contact_field.focus_set()

#set focus on the the travel_field box
def focus10(event):
    travel_field.focus_set()


#set focus on the the symptoms_field box
def focus11(event):
    symptoms_field.focus_set()
...
def clear():
    ...
    close_contact_field.delete(0, END)
    travel_field.delete(0, END)
    symptoms_field.delete(0, END)
...
def first_vaccine():
    ...
    text = df.iloc[rows - 1, 13] #Change column in index notation
...
def second_vaccine():
    ...
    LastName = df.iloc[rows - 1, 2]
    FirstName = df.iloc[rows - 1, 0]
    Text = df.iloc[rows - 1, 10]    #Change column index number as patient phone number is in different column
        ...
        if (
            ...
            if r["Sent"] == 1:
                ...
                message = client.messages.create(
                                    ...
                                    from_=Twilio_Number, #this is your own Twilio number
                                    to=r["SendText"], #this is the patient's phone number to receive the text msg, use to=round(text) if Twilio appends a 0 to your phone number.
                                    )
...
def date_verify():
    ...
    LastName = df.iloc[rows - 1, 2]
    FirstName = df.iloc[rows - 1, 0]
    Text = df.iloc[rows - 1, 13]    #Change column index
...
def insert():
    ...
    if (
        ...
        sheet.cell(row=current_row + 1, column=10).value = close_contact_field.get() 
        sheet.cell(row=current_row + 1, column=11).value = travel_field.get() 
        sheet.cell(row=current_row + 1, column=12).value = symptoms_field.get() 
        ...
        sheet.cell(row=current_row + 1, column=13).value = 1    #Send
        # add +1 to phone number input
        sheet.cell(row=current_row + 1, column=14).value = "+1" + phone_field.get() #SendText
...
def register_form():
    ...
    bg = PhotoImage(file=r"clockTower1000_500.png", height=500, width=1000)
    bg_label = Label(root, image=bg)
    bg_label.image = bg  # keep a reference!
    #bg_label.grid(row=0, column=0, columnspan=20, rowspan=20)
    bg_label.grid(row=0, column=0, columnspan=20, rowspan=50)
    ...
    global close_contact_field
    global travel_field
    global symptoms_field
    ...
    close_contact_in = StringVar()
    travel_in = StringVar()
    symptoms_in = StringVar()
    ...
    # create a label to ask about coming in contact with COVID-symptom person
    close_contact = Label(
        root, text="""Have you been in contact with anyone 
        that has been diagnosed with COVID in the past  
        14 days?""", 
        bg="black", fg="white", font=("Calibri", 13)
    )
    
    #create a label to ask about most recent travel
    travel = Label(
        root, text="""Have you travelled internationally 
        within the past 14 days?""", 
        bg="black", fg="white", font=("Calibri", 13)
    )

    #create a label for symptoms
    symptoms = Label(
        root, text="""Have you experiencned COVID symptoms 
    ...
    close_contact.grid(row=9, column=0)
    travel.grid(row=11, column=0)
    symptoms.grid(row=13, column=0)
    ...
    close_contact_field = Entry(root, textvariable=close_contact_in)
    travel_field = Entry(root, textvariable=travel_in)
    symptoms_field = Entry(root, textvariable=symptoms_in)
    ...
    close_contact_field.bind("<Return>", focus9)

    travel_field.bind("<Return>", focus10)

    symptoms_field.bind("<Return>", focus11)
    ...
    close_contact_field.grid(row=9, column=1, ipadx="100")
    travel_field.grid(row=11, column=1, ipadx="100")
    symptoms_field.grid(row=13, column=1, ipadx="100")
...
def register():
    ...
    # refrenced above
    bg = PhotoImage(file=r"logos300_420.png", height=800, width=800)
...
def login():
    ...
    bg = PhotoImage(file=r"download320_400.png", height=400, width=320)
...
def main_account_screen():
    ...
    bg = PhotoImage(file=r"sjsu_gate845_475.png", height=400, width=800)
