...
class calc:
	...
	def baseTenLog(self):
		...
		try:
			...
			self.baseTen=math.log10(self.value)
			self.e.delete(0,END)
			self.e.insert(0,self.baseTen)

	#Secodn added function
	def radConv(self):
	...
	def __init__(self,master):
			...
			# Generating Buttons
			Button(master,text="=",width=5,height=3,fg="white",
				bg="blue", font='24',command=lambda:self.equals()).grid(
									row=5, column=3)

			Button(master,text='AC',width=5,height=3,
						font='24',fg="black", bg="orange",
			command=lambda:self.clearall()).grid(row=1, column=5)


			Button(master,text='C',width=5,height=3,
				font='24',fg="black",bg="orange",
				command=lambda:self.clear1()).grid(row=1, column=4)

			Button(master,text="+",width=11,height=3, font='24',
				fg="black",bg="light green",
				command=lambda:self.action('+')).grid(row=5, column=4, columnspan=2)

			Button(master,text="x",width=11,height=3,
					fg="black",bg="light green", font='24',
					command=lambda:self.action('x')).grid(row=2, column=4, columnspan=2)

			Button(master,text="–",width=11,height=3, 
					font='24', fg="black",bg="light green",
					command=lambda:self.action('-')).grid(row=4, column=4, columnspan=2)
			...
			#Pi
			Button(master,text="π",width=6,height=3,
				fg="black",bg="light green", font='24',
				command=lambda:self.action(math.pi)).grid(row=5
                                              , column=2)

			Button(master,text=".",width=6,height=3,
				fg="black",bg="light green", font='30',
				command=lambda:self.action('.')).grid(row=5, column=0)
			...
			Button(master,text="?",width=5,height=3,
				fg="black",bg="light green", font='24',
				command=lambda:self.squareroot()).grid(row=3, column=3)

			Button(master,text="x²",width=5,height=3,
				fg="black",bg="light green", font='24',
				command=lambda:self.square()).grid(row=1, column=3)


			Button(master,text="x³",width=6,height=3,
				fg="black",bg="light green", font='24',
				command=lambda:self.thirdPower()).grid(row=1, column=2)

			#Log Base 10
			Button(master,text="log",width=5,height=3,
				fg="black",bg="light green", font='24',
				command=lambda:self.baseTenLog()).grid(row=2, column=3)
			
			#Radians
			Button(master,text="rad",width=5,height=3,
				fg="black",bg="light green", font='24',
				command=lambda:self.radConv()).grid(row=4, column=3)
